...
Sound items (music tracks) are layed out in a horizontal circular way within 80-140 degrees in front of the user, see figure \ref{fig:sounddesign}. The number of music tracks are between 3 and 10 depending on user settings, they are spatially positioned and are playing simultanously. The menu uses exocentric interaction so by turning the head toward a music track, it will move in the center of the users front audio space. It is easier to describe by using a metaphor: Imagine a carousel turning while different koncerts play around it. The user should then imagine standing in the middle of the carousel looking out. The menu design provides another interaction variant of this which includes a zoom effect. In this case the user should imagine standing near the edge of a carousel looking out. This will bring more focus to the music track in which the user is "looking" at. These two interaction mdoes are illustrated in figure \ref{fig:interactionmodes}. Parameters like the rotation degrees (80-140), music tracks (3-10) and distance (e.g. carousel size) can be adjusted according to the user preferences.

...
Spatial audio or 3D audio is constructed from binaural audio and HRTF (Head Related Transfer Functions). The word "binaural" literally just means "using both ears" and binaural recordings are reproductions of sound the way the human ear hears it. The main technology that can create these binaural recordings dynamically is HRTF. HRTF works by applying sophisticated digitally signal processing to a sound and then delivering that sound to a headset. The sound then appear as if it is coming from anywhere in space around a listener \cite{begault_3dd_1994} and this is also called spatial or 3D audio. In short this is an attempt to present sound as we normally hear it in the real world. The technology is implemented in many soundcards today.
